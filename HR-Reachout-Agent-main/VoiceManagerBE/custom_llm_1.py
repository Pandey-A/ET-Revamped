...
class CustomLLMStream(llm.LLMStream):
    ...
    async def _run(self) -> None:
        ...
        try:
            ...
            # === New: Trigger Escalation Detection ===
            try:
                analyze_payload = {
                    "user_input": user_input,
                    "assistant_response": full_response,  # you need to build this above while streaming
                    "session_id": session_id,
                    "agent_id": agent_id
                }
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.post(
                        "http://localhost:8000/chat/analyze_action",
                        json=analyze_payload
                    ) as analyze_response:
                        analyze_result = await analyze_response.json()
                        print(f"[AnalyzeAction Result] {analyze_result}")

                        if analyze_result.get("action_result", {}).get("escalated"):
                            print(f"🚨 Escalation triggered for session: {session_id}")

            except Exception as e:
                print(f"[AnalyzeAction ERROR] {e}")
