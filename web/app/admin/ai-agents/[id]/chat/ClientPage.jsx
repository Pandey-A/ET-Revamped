...
export default function AgentChatPage() {
  ...
  useEffect(() => {
    if (!agent || !id) return;
    if (!isAgentChatReady(agent)) {
      toast.info("Chat is available after knowledge base indexing completes.");
      router.replace(`/admin/ai-agents/${id}/resources`);
    }
  }, [agent, id, router, pendingCount]);
  ...
  // ── User websocket for escalation events and human support replies
  useEffect(() => {
    if (!sessionId) return;
      ...
      socket.onmessage = (event) => {
        ...
        try {
          ...
          if (data.escalated === true) {
            setIsEscalated(true);
            toast.info("You're now connected to human support.");
            return;
          }
  ...
  if (!agent || !isAgentChatReady(agent)) {
    return (
      <div className="admin-page admin-page--status">
        <Navbar />
        <div className="admin-nav-spacer" />
        <div className="admin-status-card">{agent ? 'Checking knowledge base…' : 'Loading chat…'}</div>
      </div>
    );
  }
          ...
          {sidebarOpen && (
            ...
            >
              ...
              <div className="aia-chat-sidebar-section">
                <p className="aia-chat-sidebar-label">Documents</p>
                <p className="aia-chat-sidebar-value">
                  {agent.resource_list?.length || 0} source{agent.resource_list?.length !== 1 ? "s" : ""}
                </p>
              </div>
        ...
        <div className="aia-chat-main">
          ...
          <div className="aia-chat-topbar">
            ...
            <div className="aia-chat-topbar-right">
              {isEscalated && (
                <span className="aia-offline-badge">Human support</span>
              )}
              {!connected && (
                <span className="aia-offline-badge">Offline</span>
              )}
            </div>
          </div>
            ...
            {messages.length === 0 && (
              <div className="aia-chat-welcome">
                ...
                <div className="aia-chat-welcome-hints">
                  ...
                  <span>📚 Answers use your agent&apos;s knowledge base</span>
                </div>
              </div>
            )}
          ...
          <div className="aia-chat-input-bar">
            ...
            {isEscalated ? (
              <p className="aia-chat-input-hint">
                Your messages are being handled by a human team member.
              </p>
            ) : null}
          </div>
        </div>
