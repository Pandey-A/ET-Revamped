...
export async function POST(request) {
  ...
  try {
    const res = await fetch(`${aiBase()}/widget/session/contact`, {
      method: 'POST',
    ...
    if (!res.ok) {
      return Response.json({ error: data.detail || 'Contact update failed' }, { status: res.status, headers: corsHeaders() });
    }
    return Response.json({ status: 'ok', session: data.session }, { headers: corsHeaders() });
  } catch (e) {
    console.error('[api/widget/contact]', e);
