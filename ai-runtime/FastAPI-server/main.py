...
def _billing_user_for_agent(agent_id: Optional[str]) -> Optional[str]:
    if not agent_id:
        return None
    owner = credits_store.get_agent_owner(agent_id)
    if owner:
        return owner
    agent_cfg = _get_agent_config(agent_id)
    owner = (agent_cfg.get("owner_user_id") or "").strip()
    if owner:
        credits_store.set_agent_owner(agent_id, owner)
        return owner
    return None
...
async def create_whatsapp_channel(payload: WhatsAppChannelCreate):
    ...
    body = _core_api_request(
        ...
        json_body={
            ...
            "ai_agent_name": payload.ai_agent_name or "",
            "admin_phone": payload.admin_phone or "",
            **(
                {"config_json": payload.config_json.model_dump(exclude_none=True)}
...
async def update_whatsapp_channel(id: str, payload: WhatsAppChannelUpdate):
    ...
    if "config_json" in cleaned_updates and isinstance(cleaned_updates["config_json"], dict):
        cleaned_updates["config_json"] = {
            k: v for k, v in cleaned_updates["config_json"].items() if v is not None
        }
...
async def upload_agent_data(payload: AgentStorePayload):
    try:
        agent_data = payload.model_dump(exclude_unset=False)
...
async def get_credits_tokens(user_id: str):
    """Per-session LLM token usage (ai-runtime/data parity)."""
    if not user_id or not user_id.strip():
        raise HTTPException(status_code=400, detail="user_id is required")
    uid = user_id.strip()
...
async def index_url_to_qdrant_endpoint(request: IndexUrlRequest, background_tasks: BackgroundTasks):
    try:
        logging.info(f"📥 Full request: {request.model_dump()}")
...
async def index_file_to_qdrant(
    ...
    try:
        IndexPdfMeta(collection_name=collection_name, agent_id=agent_id)
        if not file.filename:
            return JSONResponse(status_code=422, content={"error": "PDF file is required"})
    ...
    try:
        if os.path.exists(AGENTS_DB):
            ...
            for agent in agents:
                ...
                owner = (agent.get("owner_user_id") or "").strip()
                if aid and owner:
                    credits_store.set_agent_owner(aid, owner)
            logging.info("[Credits] Synced agent owner mappings from %s", AGENTS_DB)
    except Exception as e:
        logging.warning("[Credits] Agent owner backfill skipped: %s", e)
