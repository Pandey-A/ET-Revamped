...
def _react_agent_supports_system_prompt() -> bool:
    """LlamaIndex 0.14+ uses system_prompt; 0.12.x uses context via from_tools."""
    return "system_prompt" in inspect.signature(ReActAgent.__init__).parameters
...
class CoreAgentHandler:
    ...
    def create_core_agent(
        ...
        if _react_agent_supports_system_prompt():
            return ReActAgent(
                tools=tools,
                llm=llm,
    ...
    def run_agent_sync(agent: ReActAgent, user_input: str):
        """Run agent from a worker thread (chat + WhatsApp handlers)."""
...
def extract_agent_reply(chat_response) -> str:
    """Normalize LlamaIndex agent responses to plain text."""
